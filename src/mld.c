...
static void add_object_to_object_db(object_db_t *object_db, void *ptr, int units, struct_db_rec_t *struct_rec, mld_boolean_t is_root)
{
    ...
    assert(!obj_rec); // Dont add same object twice
...
// The global object of the application which is not created by xcalloc should be registered with MLD using below API
...
static void mld_explore_objects_recursively(object_db_t *object_db, object_db_rec_t *parent_obj_rec)
{
    ...
    for (i = 0; i < parent_obj_rec->units; i++)
    {
        ...
        for (n_fields = 0; n_fields < parent_struct_rec->n_fields; n_fields++)
        {
            ...
            // We are only concerned with fields which are pointer to other objects
    ...
    for (; obj_index < units; obj_index++)
    {
        ...
        for (field_index = 0; field_index < n_fields; field_index++)
        {
            ...
            case DOUBLE:
                ...
                break;
            case OBJ_PTR:
                printf("%s[%d]->%s = %p\n", obj_rec->struct_rec->struct_name, obj_index, field->fname, (void *)(*(size_t *)(current_object_ptr + field->offset)));
                break;
            case OBJ_STRUCT:
                /*Later*/
                break;
