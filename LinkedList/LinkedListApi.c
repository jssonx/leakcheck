...
void init_register_linked_list_structures_with_mld(object_db_t *_object_db)
{

    object_db = _object_db;

    /*Write code to register linkedlist structures:
     * singly_ll_node_t
     * ll_t
     * */
    static field_info_t singly_ll_node_t_fields[] = {
        /* Here we are supporting a new data type called VOID_PTR. Structures/Data types of void* objects are not known,
         * therefore, MLD algorithm cannot expore the child objects of void * objects.
         * You should update your mld algorithm to handle this scenario as a part of this assignment.*/
        FIELD_INFO(singly_ll_node_t, data, VOID_PTR, 0),
        FIELD_INFO(singly_ll_node_t, next, OBJ_PTR, singly_ll_node_t)};
    REG_STRUCT(object_db->struct_db, singly_ll_node_t, singly_ll_node_t_fields);

    static field_info_t ll_t_fields[] = {
        FIELD_INFO(ll_t, node_count, UINT32, 0),
        FIELD_INFO(ll_t, head, OBJ_PTR, singly_ll_node_t)};
    REG_STRUCT(object_db->struct_db, ll_t, ll_t_fields);
}

ll_t *init_singly_ll()
{
    // return calloc(1, sizeof(ll_t));
    return xcalloc(object_db, "ll_t", 1);
}

singly_ll_node_t *singly_ll_init_node(void *data)
{
    singly_ll_node_t *node = xcalloc(object_db, "singly_ll_node_t", 1);
    node->data = data;
    return node;
}
...
int singly_ll_delete_node(ll_t *ll, singly_ll_node_t *node)
{
    ...
    if (node->next)
    {
        ...
        /*ToDo : replace below free by xfree*/
        xfree(object_db, temp);
